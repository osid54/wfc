...
@export var waittime := 32
@export var tile := preload("res://things/tileDict.tscn")
var map := []
var mapData := []
var numCollapsed := 0
...
#			if map[x+i][y+j].tileType=="":
#				map[x+i][y+j].narrow(t)

	if x!=0 and map[x-1][y].tileType=="":
		map[x-1][y].narrow(t)
	if x!=size-1 and map[x+1][y].tileType=="":
		map[x+1][y].narrow(t)
	if y!=0 and map[x][y-1].tileType=="":
		map[x][y-1].narrow(t)
	if y!=size-1 and map[x][y+1].tileType=="":
		map[x][y+1].narrow(t)
